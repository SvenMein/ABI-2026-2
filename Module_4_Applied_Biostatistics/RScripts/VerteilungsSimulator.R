...
tempplot +
  geom_area(
    data = d %>% filter(x >= chi2_threshold5, x <= 15),
    aes(x, y),
    fill = "orangered", alpha = .5
  ) +
  annotate(
    geom = "label", x = chi2_threshold5, y = .1,
    label = "AUC=0.05",
    hjust = 0
  )
...
tempplot +
  geom_area(
    data = d %>% filter(x >= 160, x <= 180), aes(x, y),
    fill = "lightgreen", alpha = .5
  ) +
  geom_area(
    data = d %>% filter(x < 160, x >= 150), aes(x, y),
    fill = "yellow", alpha = .5
  ) +
  geom_area(
    data = d %>% filter(x > 180, x <= 190), aes(x, y),
    fill = "yellow", alpha = .5
  ) +
  geom_area(
    data = d %>% filter(x < 150), aes(x, y),
    fill = "orangered", alpha = .5
  ) +
  geom_area(
    data = d %>% filter(x > 190), aes(x, y),
    fill = "orangered", alpha = .5
  ) +
  geom_vline(xintercept = c(160, 180), color = "grey") +
  annotate(
    geom = "label", x = 180, y = .035,
    label = "Mean + SD\n170+10",
    hjust = 0
  ) +
  annotate(
    geom = "label", x = 160, y = .035,
    label = "Mean - SD\n170-10",
    hjust = 1
  )
# "true" threshold for 5%:
