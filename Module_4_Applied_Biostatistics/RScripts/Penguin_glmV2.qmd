...
```{r anatomy_model}
pred_vars <- ColSeeker(namepattern = c("bill","fl","_hg")) #hg
glmformula <- reformulate(pred_vars$names, response = "sex")
logreg_out<-glm(glmformula,
                family = binomial(), data = rawdata)
logreg_out |> 
  tidy() |> 
  mutate(
    estimate = exp(estimate) |> roundR(10),
    p.value = formatP(p.value,5)) |> 
  flextable() |> 
...
SensSpecComparison <- 
  add_row(
    ...
    Sensitivity = youden$sensitivity,
    Specificity = youden$specificity,
    AIC = AIC(logreg_a_out) |> 
      roundR(5))
ggroc(roc_out_a,legacy.axes = TRUE) + 
  geom_abline(slope = 1,intercept = 0) + 
  geom_point(x = 1 - youden$specificity,
             y = youden$sensitivity, color = 'red', size = 2) 
...
for(var_i in pred_vars$names){
  for(species_effect in c("","+", "*")){
    ...
    AIC_out <- AIC(logreg_out) |> roundR(5)
